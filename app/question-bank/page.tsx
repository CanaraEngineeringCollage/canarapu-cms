...
interface AddQuestionPaperModalProps {
 open: boolean;
 onClose: () => void;
 onSubmit: (name: string, subject: string, year: string, file: File) => void;
 loading: boolean;
}
...
const AddQuestionPaperModal = ({ open, onClose, onSubmit, loading }: AddQuestionPaperModalProps) => {
 ...
 useEffect(() => {
  if (!open) { setName(""); setSubject(""); setYear(""); setFile(null); }
 }, [open]);
...
const QuestionBankPage = () => {
 ...
 const uploadQuestionPaper = async (name: string, subject: string, year: string, file: File) => {
  setUploading(true);
  try {
